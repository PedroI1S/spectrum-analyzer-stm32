...
static void display_thread(void *a, void *b, void *c)
{
	...
	while (1) {
		...
		/* TODO(F1/F4): desenhar no OLED SSD1306 (chosen zephyr,display + cfb).
		 * Sem o display, imprime um resumo no console a ~2 Hz. */
		...
		case MODE_SPECTRUM: {
			...
			line[N_BARS] = '\0';
			LOG_INF("[spectrum] %s  dom=%u Hz", line, f.dominant_hz);
			break;
		}
		case MODE_WAVE:
			LOG_INF("[wave] frame %u (forma de onda - TODO F4)", f.seq);
			break;
		default:
			LOG_INF("[stats] fps=%u fft=%u us max=%u us overruns=%u",
...
int main(void)
{
	...
	LOG_INF("shell: 'kernel thread list', 'rt status', 'rt mode <spectrum|wave|stats>'");
	return 0;
}
