...
- ✅ **F3 (pitch/afinador) concluída** — detector de nota por **autocorrelação** na thread hard-RT: janela deslizante de 1024 amostras (64 ms), primeiro pico ≥85% do máximo (robusto a harmônicos), interpolação parabólica (precisão sub-amostra), gate de nível + gate de qualidade. **Fs calibrada = 16039,3 Hz** (o prescaler do I2S arredonda: PLLI2S 135,5 MHz/132/64 — usar 16000 nominal daria ~4 cents de erro sistemático). Modo `tuner` no display (nota, Hz, cents e barra de afinação) + comando `rt tune [s]`. Lição de RT: janela de 2048 estourava o deadline (ACF ~43 ms > bloco de 32 ms) → reduzida para 1024. Validado com instrumento: estável, 0 overruns.
  - **Faixa de detecção (validada empiricamente): ~E2 até D7** (82–2349 Hz; teto teórico 2670 Hz com lag mínimo = 6 amostras). Na borda do alcance a autocorrelação **erra por uma oitava** (o pico do período real cai fora da janela de busca e o detector pega o de 2× o período): foi observado com lag mín = 12 (teto 1337 Hz), onde E6 = 1319 Hz era lido como 659 Hz → corrigido estendendo o lag mínimo para 6. Nos agudos (>1,5 kHz) a precisão em cents degrada por natureza (período de poucas amostras); a faixa útil para afinar instrumento (80–1000 Hz) mantém precisão cheia.
...
1. **Include errado de clock no overlay:** incluir `stm32_clock.h` (genérico) junto com `stm32f4_clock.h` faz `STM32_CLOCK_BUS_APB1` expandir para `0x2` em vez de `0x040` → `clock_control_on` escreve no registrador errado → `i2s_stm32_initialize: clock enabling failed: -5` e o device fica not-ready (`-ENODEV` no app). **Incluir apenas `stm32f4_clock.h`** (o mesmo que o SoC dtsi usa).
2. **Amostras partidas em metades de 16 bits:** o registrador de dados do I2S no F4 é de 16 bits; em word_size 24/32 cada slot chega em duas metades, e o DMA (configurado em words de 32 pelo driver) grava **cada metade zero-estendida num word próprio**. Layout real por frame: `[Lhi, Llo, Rhi, Rlo]` (4 words). Sintomas: taxa 2×, padrão `[dado, dado, 0, 0]`, nível preso em 255. **Correção no app:** recombinar `((hi<<16)|lo) >> 8` → 24 bits com sinal (ver `acq_fft_thread`). Diagnóstico com o comando `rt dump` (amostras cruas) + analisador lógico (confirmou 24 bits no barramento).
